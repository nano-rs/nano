-- ============================================================================
-- Migration 052: Update Squid Proxy parser to extract referrer from log line
-- ============================================================================
-- The squid proxy log generator now outputs extended format with referrer:
-- timestamp duration client_ip result/status bytes method url user hierarchy content_type "referrer"
--
-- This migration updates the parser_vrl to extract the referrer field from
-- the quoted value at the end of the log line for web session tree visualization.
-- ============================================================================

UPDATE log_sources
SET parser_vrl = $$
# Squid Proxy Access Log Parser
# Standard format: timestamp duration client_ip result/status bytes method url user hierarchy content_type
# Extended format: timestamp duration client_ip result/status bytes method url user hierarchy content_type "referrer"
# The referrer field is optional and quoted - used for web session tree visualization
...
if length(parts) >= 10 {
    ...
    if exists(url_match.host) {
        .udm.dest_host = to_string(url_match.host)
        .udm.url_domain = to_string(url_match.host)
    }
    ...
    if http_method == "CONNECT" {
        ...
        if exists(connect_match.host) {
            .udm.dest_host = to_string(connect_match.host)
        }
    ...
    # Extended format ends with: "referrer_url" or "-"
