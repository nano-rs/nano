...
impl PrevalenceSettings {
    ...
    /// Update only the rarity threshold
    ///
    /// Requirement: 8.1
    pub async fn set_rarity_threshold(
        &self,
        threshold: u64,
    ) -> Result<(), PrevalenceSettingsError> {
        if threshold < 1 || threshold > 1000 {
            return Err(PrevalenceSettingsError::InvalidRarityThreshold(threshold));
        }

        sqlx::query(
            "UPDATE system_settings SET prevalence_rarity_threshold = $1, updated_at = NOW() WHERE id = 'default'"
        )
        .bind(threshold as i32)
        .execute(&self.pool)
        .await?;

        info!("Prevalence rarity threshold updated to {}", threshold);
        Ok(())
    }

    /// Enable or disable hash tracking
    ///
    /// Requirement: 8.2
    pub async fn set_hash_tracking_enabled(
        &self,
        enabled: bool,
    ) -> Result<(), PrevalenceSettingsError> {
        sqlx::query(
            "UPDATE system_settings SET prevalence_enable_hash_tracking = $1, updated_at = NOW() WHERE id = 'default'"
        )
        .bind(enabled)
        .execute(&self.pool)
        .await?;
        ...
        info!(
            "Prevalence hash tracking {}",
            if enabled { "enabled" } else { "disabled" }
        );
        Ok(())
    }

    /// Enable or disable domain tracking
    ///
    /// Requirement: 8.2
    pub async fn set_domain_tracking_enabled(
        &self,
        enabled: bool,
    ) -> Result<(), PrevalenceSettingsError> {
        sqlx::query(
            "UPDATE system_settings SET prevalence_enable_domain_tracking = $1, updated_at = NOW() WHERE id = 'default'"
        )
        .bind(enabled)
        .execute(&self.pool)
        .await?;
    ...
    ///
    /// Requirement: 8.3
    pub async fn set_retention_days(&self, days: u32) -> Result<(), PrevalenceSettingsError> {
        if days < 1 || days > 365 {
            return Err(PrevalenceSettingsError::InvalidRetentionDays(days));
        }

        sqlx::query(
            "UPDATE system_settings SET prevalence_retention_days = $1, updated_at = NOW() WHERE id = 'default'"
        )
        .bind(days as i32)
        .execute(&self.pool)
        .await?;

        info!("Prevalence retention period updated to {} days", days);
        Ok(())
    }
    ...
    ///
    /// Requirement: 8.4
    pub async fn set_cache_ttl_seconds(&self, ttl: u64) -> Result<(), PrevalenceSettingsError> {
        if ttl > 3600 {
            return Err(PrevalenceSettingsError::InvalidCacheTtl(ttl));
        }

        sqlx::query(
            "UPDATE system_settings SET prevalence_cache_ttl_seconds = $1, updated_at = NOW() WHERE id = 'default'"
        )
        .bind(ttl as i32)
        .execute(&self.pool)
        .await?;

        info!("Prevalence cache TTL updated to {} seconds", ttl);
        Ok(())
    }
}
