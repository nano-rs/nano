...
impl DetectionService {
    /// Create a new detection service with PostgreSQL only (legacy mode)
    ///
    /// In this mode, both metadata and log queries use PostgreSQL.
    pub fn new(pool: PgPool) -> Self {
        ...
        Self {
            ...
            pg_pool: pool.clone(),
            prevalence_evaluator: None,
            risk_weight_cache: Arc::new(RwLock::new(None)),
            version_manager: RuleVersionManager::new(pool),
    ...
    /// Create a new detection service with custom configuration (PostgreSQL only)
    pub fn with_config(pool: PgPool, config: DetectionServiceConfig) -> Self {
    ...
    /// Create a new detection service with DualPool (ClickHouse for logs, PostgreSQL for metadata)
    ///
    /// This is the recommended mode for production use:
    /// - PostgreSQL is used for rule and alert storage (Requirements 6.2, 6.3)
    /// - ClickHouse is used for log queries via SearchService (Requirement 6.1)
    ///
    /// Note: This constructor does NOT include prevalence support. Use `with_dual_pool_and_prevalence`
    /// if you need prevalence-based detection filtering.
    pub fn with_dual_pool(dual_pool: &DualPool) -> Self {
        ...
        Self {
            ...
            search_service: SearchService::with_dual_pool(dual_pool),
            finding_logger: Some(finding_logger),
            config: DetectionServiceConfig::default(),
            score_calculator: ScoreCalculator::new(),
    ...
    /// Create a new detection service with DualPool and custom configuration
    pub fn with_dual_pool_and_config(dual_pool: &DualPool, config: DetectionServiceConfig) -> Self {
        let finding_logger = if config.signal_logging_enabled {
    ...
    /// Create a new detection service with a pre-configured SearchService
    ///
    /// This allows for maximum flexibility in configuring the search backend,
    /// including lookup table support.
