...
///
/// Unset and empty both mean `None`. A non-empty but malformed value is a hard
/// error rather than a fallback: a typo in a real production key must never
/// silently downgrade a release to the publicly-known DEV placeholder.
///
...
fn generate_airgap_pubkey(out_dir: &str) {
    ...
    // Shared by the "not set" and "set but empty" arms — both mean the same
    // thing to `airgap/mod.rs`: fall back to the DEV placeholder, and let the
    // release-build guard refuse to trust it.
    let not_injected = || {
        format!(
            "// @generated by build.rs: {} was not set (or was empty).\n\
    ...
    let generated = match env::var(AIRGAP_PUBKEY_ENV) {
        // NAN-2299: an EMPTY value means "not injected", not "invalid".
        // `Dockerfile:56-57` declares `ARG AIRGAP_BUNDLE_PUBLIC_KEY_HEX=""` and
        // promotes it to ENV unconditionally, so inside ANY container build this
        // variable is always set — which makes the `NotPresent` arm below
        // unreachable there. Without this arm a plain `docker build`, with no
        // build-arg, panics instead of producing the unsigned build documented
        // above. Our own CI never noticed because the workflow passes
        // `vars.AIRGAP_BUNDLE_PUBLIC_KEY_HEX` at every build site.
        Ok(hex) if hex.trim().is_empty() => not_injected(),
        Ok(hex) => {
        ...
        Err(env::VarError::NotPresent) => not_injected(),
